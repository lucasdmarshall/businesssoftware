...
- Phase 4 attendance depth completed: leave balances (entitlement + approval-driven used days), attendance correction history, shift status updates, and payroll rules with worked/overtime hour summaries (migration `021_leave_balance_corrections_payroll.sql`)
- Cross-device attendance continues to merge on `(organization, user, work_date)`; corrections append history and audit events rather than silently overwriting

Next actions:

1. Add department data policies beyond the existing own/team/department/organization scopes.
2. Wire leave (and remaining module-local approve/reject paths) through the generic workflow engine.
3. Add workflow approval reminders/deadlines and delegated approval.
4. Add export audit hooks when reporting/export services are introduced.
5. Continue Phase 7 HR depth: employee profiles, onboarding, offboarding, HR documents.
6. Continue Phase 7 Finance depth: invoices, budgets, purchase requests.
7. Finish automated device-offline harness testing.
8. Unblock macOS `.dmg` packaging.
