...
- Phase 7 business modules completed across all domains (migrations `022`–`025`): Finance now adds invoices, budgets (spend derived from paid expenses), and purchase requests (workflow-approved); HR adds employee profiles, onboarding checklists, and HR documents; Sales/CRM adds companies, contacts, leads, opportunities with a pipeline stage, and an activity log; IT/Ops adds tickets (covering service, access, and incident requests), an asset registry, and a knowledge base
...
1. Land Schedule production PR (renumber its migration to `031` if Finance suite merges first), then Finance depth (aging reports, bank recon), or Department workspaces rebase.
...
>>>>>>> 83bebd2 (Turn Finance into a full suite with ledger, AP, AR, and cash.)
