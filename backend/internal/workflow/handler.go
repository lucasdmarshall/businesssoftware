...
func (h Handler) Instances(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Query().Get("overdue") == "1" {
		filters += ` AND i.status='in_review' AND i.due_at IS NOT NULL AND i.due_at < NOW()`
	}
	query := `
...
func (h Handler) SetDue(w http.ResponseWriter, r *http.Request) {
	...
	if json.NewDecoder(r.Body).Decode(&input) != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "due_at is required (RFC3339) or null to clear"})
		return
	}
	var submittedBy, status string
	if err := h.DB.QueryRow(r.Context(), `SELECT submitted_by, status FROM workflow_instances WHERE id=$1 AND organization_id=$2`, instanceID, user.OrganizationID).Scan(&submittedBy, &status); err != nil {
		writeJSON(w, http.StatusNotFound, map[string]string{"error": "request not found"})
		return
	}
	if status != "in_review" {
		writeJSON(w, http.StatusConflict, map[string]string{"error": "deadline can only be set while in review"})
		return
	}
	canManage := h.Auth.HasPermission(r.Context(), user, "workflow.manage")
	if submittedBy != user.ID && !canManage {
		writeJSON(w, http.StatusForbidden, map[string]string{"error": "only the submitter or a workflow manager can set the deadline"})
		return
	}
	var due any
	if input.DueAt != nil && strings.TrimSpace(*input.DueAt) != "" {
		parsed, err := time.Parse(time.RFC3339, strings.TrimSpace(*input.DueAt))
		if err != nil {
			writeJSON(w, http.StatusBadRequest, map[string]string{"error": "due_at must be RFC3339"})
			return
		}
		due = parsed.UTC()
	}
	_, err = h.DB.Exec(r.Context(), `UPDATE workflow_instances SET due_at=$1, last_reminded_at=NULL, updated_at=NOW() WHERE id=$2`, due, instanceID)
	if err != nil {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "could not set deadline"})
		return
	}
	_, _ = h.DB.Exec(r.Context(), `INSERT INTO workflow_actions (instance_id, actor_id, action, reason) VALUES ($1,$2,'deadline',$3)`,
		instanceID, user.ID, "deadline updated")
	h.audit(r.Context(), user, "workflow.deadline_set", "workflow_instance", instanceID, map[string]any{"due_at": input.DueAt})
	writeJSON(w, http.StatusOK, map[string]any{"status": "updated", "due_at": input.DueAt})
}
