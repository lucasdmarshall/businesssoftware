-- Phase 5 production depth: delegated approval, step SLAs / instance deadlines,
-- and reminder log. Leave already links via migration 027.
...
ALTER TABLE workflow_actions ADD CONSTRAINT workflow_actions_action_check
    CHECK (action IN ('submit', 'approve', 'reject', 'resubmit', 'cancel', 'comment', 'delegate', 'remind', 'deadline'));
...
CREATE INDEX IF NOT EXISTS workflow_instances_due_idx
    ON workflow_instances (organization_id, status, due_at)
    WHERE due_at IS NOT NULL AND status = 'in_review';
